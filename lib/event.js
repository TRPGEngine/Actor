...
exports.getTemplate = async function(data, cb, db) {
  ...
  if(!uuid || typeof uuid !== 'string') {
    ...
    let templates = await user.getTemplatesAsync();
    templates = templates.filter(i => i.is_deleted === false);
    return {templates};
  }else {
    // 返回指定模板信息
...
exports.createTemplate = function(data, cb) {
  ...
  try {
	...
	if(!name) {
      cb({result: false, msg: '缺少模板名'});
	  return;
	}

	if(!info) {
      cb({result: false, msg: '缺少模板信息'});
	  return;
	}
...
exports.createTemplateAdvanced = async function(data, cb, db) {
  ...
  let isExist = await db.models.actor_template.existsAsync({name});
  if(isExist) {
    throw '该模板名字已存在';
  }

  let template = await db.models.actor_template.createAsync({
    name,
    desc,
    avatar,
    info,
  });
  await template.setCreatorAsync(player.user);
